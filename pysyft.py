...
def locked_data():
    federated_train_loader = sy.FederatedDataLoader(
        df.load_mnist[0].federate((kunal, aayush, saatvik)), batch_size=128, shuffle=True) # the federate() method splits the data within the workers

    test_loader = torch.utils.data.DataLoader(
        df.load_mnist[1], batch_size=128, shuffle=True)

    train_loader = torch.utils.data.DataLoader(
        df.load_mnist[0], batch_size=128, shuffle=True)
